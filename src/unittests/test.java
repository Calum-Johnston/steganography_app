...
public class test {
    ...
    public void runTests() {
                        ...
                        for(boolean red : new boolean[] {false, true}) {
                            for (boolean green : new boolean[]{false, true}) {
                                for (boolean blue : new boolean[]{false, true}) {
                                    ...
                                    stegoImage = encoder.encode(coverImage, 0, red, green, blue,
                        ...
                        for(boolean red : new boolean[] {false, true}) {
                            for (boolean green : new boolean[]{false, true}) {
                                for (boolean blue : new boolean[]{false, true}) {
                                    ...
                                    stegoImage = encoder.encode(coverImage, 1, red, green, blue,
                                            redBits, greenBits, blueBits, random, seed, text);
                                    String assertionError = null;
                                    try {
                                        result = decoder.decode(stegoImage);
            ...
            for(boolean red : new boolean[] {false, true}) {
                for (boolean green : new boolean[]{false, true}) {
                    for (boolean blue : new boolean[]{false, true}) {
                        ...
                        stegoImage = encoder.encode(coverImage, 2, red, green, blue,
                                redBits, greenBits, blueBits, random, seed, text);
                        String assertionError = null;
                        try {
                            result = decoder.decode(stegoImage);
        ...
        // PVD
        for(redBits = 1; redBits < 2; redBits++){
            for(greenBits = 1; greenBits < 2; greenBits++){
                for(blueBits = 1; blueBits < 2; blueBits++){
